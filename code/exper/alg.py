...
BetaTypes1 = tools.make_enum("BetaTypes1", "DaiYuan PolakRibiere Hybrid1 Hybrid2 Hybrid3".split())


class ConjugateGradient1(Solver):
    """
    Module containing conjugate gradient algorithm based on
    conjugategradient.m from the manopt MATLAB package.
    """

    def __init__(self, beta_type=BetaTypes1.DaiYuan, orth_value=np.inf, linesearch=None, *args, **kwargs):
        """
        Instantiate gradient solver class.
        Variable attributes (defaults in brackets):
            - beta_type (BetaTypes1.HestenesStiefel)
    ...
    def solve(self, problem, x=None, reuselinesearch=False):
        ...
        while True:
            ...
            if abs(orth_grads) >= self._orth_value:
                ...
                if self._beta_type == BetaTypes1.DaiYuan:
                    diff = newgrad - oldgrad    #y_{k-1}
                    beta = newgradPnewgrad / man.inner(newx, diff, desc_dir)
                elif self._beta_type == BetaTypes1.PolakRibiere:
                    diff = newgrad - oldgrad
                    ip_diff = man.inner(newx, Pnewgrad, diff)
                    beta = ip_diff / gradPgrad
                elif self._beta_type == BetaTypes1.Hybrid1:
                    diff = newgrad - oldgrad
                ...
                elif self._beta_type == BetaTypes1.Hybrid2:
                    diff = newgrad - oldgrad
                    beta_DY = newgradPnewgrad / man.inner(newx, diff, desc_dir)
                    ip_diff = man.inner(newx, Pnewgrad, diff)
                    try:
                ...
                elif self._beta_type == BetaTypes1.Hybrid3:
                    diff = newgrad - oldgrad
                    #beta_DY = newgradPnewgrad / man.inner(newx, diff, desc_dir)
                    #ip_diff = man.inner(newx, Pnewgrad, diff)
                    #man.norm(newx, newgrad)/man.norm(newx, oldgrad)*man.inner(newx, Pnewgrad, desc_dir),
                    beta= (newgradPnewgrad - max(0, man.norm(newx, newgrad)/man.norm(newx, oldgrad)*man.inner(newx, oldgrad, Pnewgrad)))/max(pow(man.norm(newx, oldgrad),2), man.inner(newx, diff, desc_dir))
                    #try:
                    #    beta_HS = ip_diff / man.inner(newx, diff, desc_dir)
                    #except ZeroDivisionError:
                    #    beta_HS = 1
                    #sk=newx-x  #S_{k-1}
                    #sk=newx-man.transp(x, newx, x)
                    #sk=-stepsize*oldgrad
                    #theta= man.inner(newx, sk, Pnewgrad)/ man.inner(newx, oldgrad, Pnewgrad)
                
                    #eta = (1-theta)*beta_HS+theta*beta_DY
                    #beta =0.002*newgrad/desc_dir
                elif self._beta_type == BetaTypes1.Hybrid4:
                    diff = newgrad - oldgrad
                    beta_DY =newgradPnewgrad / man.inner(newx, diff, desc_dir)
                    ip_diff = man.inner(newx, Pnewgrad, diff)
                    try:
                        ...
                        beta_HS = 1
                    #beta = max((abs(man.inner(newx, Pnewgrad, oldgrad)) / (-man.inner(newx, oldgrad, desc_dir))) * beta_DY, min(beta_DY, beta_HS))    
                    c2 = linesearch.c2
                    c=-(1 - c2) / (1 + c2)
                    #the=(1+c)*newgradPnewgrad/((1+c)*newgradPnewgrad-2*man.inner(newx, Pnewgrad, oldgrad))
                    #beta=(1-pow(the,2))*beta_HS+the/2*((1+c)* beta_DY+the*(1-c)*beta_DY)
                    beta=min(abs(beta_HS),(newgradPnewgrad - man.norm(newx, newgrad)/man.norm(newx, oldgrad)*man.inner(newx, oldgrad, Pnewgrad))/(pow(man.norm(newx, oldgrad),2)+3*abs(man.inner(newx, oldgrad, Pnewgrad))))
                else:
                    types = ", ".join(["BetaTypes1.%s" % t for t in BetaTypes1._fields])
            ...
            gradnormm.append(cost)
            gradPgrad = newgradPnewgrad

            iter += 1
        
        return x, iter + 1, time.time() - time0,  gradnormm
...
class LineSearchWolfe:
    ...
    def search(self, objective, man, x, d, f0, df0, gradient):
        ...
        newx = man.retr(x, stepsize * d) #x_{k+1}
        gradz = gradient(newx)
        y=gfk-man.transp(x, newx, gradz)
        #y=gradz-man.transp(x, newx, gfk)
        #y=gfk-man.transp(x, newx, gradz)
        aa=stepsize*man.inner(newx, gfk, d)
        bb=-stepsize*man.inner(newx, y, d)
        if bb!=0:
           newx=man.retr(x, -aa/bb*stepsize * d)
        else:
           newx=newx
...
def _scalar_search_wolfe(phi, derphi, c1=1e-4, c2=0.9, maxiter=100):
    ...
    for i in range(maxiter):
        if (phi_a1 > phi0 + c1 * alpha1 * derphi0) or ((phi_a1 >= phi_a0) and (i > 1)):
            ...
            break
    #(abs(derphi_a1) <= c2 * abs(derphi0)): 
    # (c2 * derphi0<=derphi_a1 and derphi_a1<=2*(c1-1)*derphi0):   #广义的Wolfe线搜索
        derphi_a1 = derphi(alpha1)
        if (c2 * derphi0<=derphi_a1 and derphi_a1<=(2*c1-1)*derphi0):   #广义的Wolfe线搜索
            alpha_star = alpha1
            phi_star = phi_a1
